...
- CI 服务方面，熟悉 `Travis CI`、`Scrutinizer CI`、`Style CI`、`Coveralls`、`Sentry`。
...
目前我在此公司任职 PHP 工程师，公司业务方向为电商小程序，本人主要负责项目如下。
...
虽然，此项目中途被砍，但整个后端从零到基本架构到各类服务到部署脚本到 HTTP API，全是我一手编写，进行了大量的前期工作、具备一定技术难度，所以还是决定放上简历，面试时可展示代码。
...
在前端兼容微信、支付宝、百度小程序的过程中，由于许多关键字不同、配置不同、文件后缀不同，所以我根据前端的需求，利用 `Symfony/Console` 编写了简易的转换脚本，成功实现一套代码兼容三套平台，大大减轻前端工作量。
